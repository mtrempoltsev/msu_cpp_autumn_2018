...
public:
    ...
    pointer alloc(size_type num) {
        return static_cast<pointer>(::operator new[](num * sizeof(size_type)));
    }

    void dealloc(pointer ptr, size_type num) {
...
public:
    ...
    explicit Vector(size_type count = 0)
            ...
            , data{alloc.alloc(cur_alloc_size)}
            , cur_used_size{count}
    {
