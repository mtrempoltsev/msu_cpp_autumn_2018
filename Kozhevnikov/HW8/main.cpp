...
public:
    ...
    void *reallocate(T *obj, size_t size) {
        T *buf = new T[size];
        if (size > size_) {
            std::memcpy(buf, obj, size_ * sizeof(T));
            size_ = size;
        } else {
            std::memcpy(buf, obj, size * sizeof(T));
            size_ = size;
        }
        return buf;
    }
...
public:
    Iterator(T &&begin, T &&end, int dir = 1)
            : current_(std::move(begin)), end_(std::move(end)), dir(dir) {
        findNext(dir);
    }
...
public:
    ...
    void reserve(size_t res_size) {
        if (res_size > size_val) {
            alloc_.reallocate(data, res_size);
            size_val = res_size;
        }
    };
    ...
    void push_back(T val) {
        ...
        alloc_.reallocate(data, ++size_val);
        data[size_val - 1] = val;
    };

    T pop_back() {
        T buf = data[size_val - 1];
        alloc_.reallocate(data, --size_val);
        if (size_val == 0) {
            empty_val = true;
        }
    ...
    iterator begin() {
        return Iterator<typename Vector::iterator>(data[0], data[size_val - 1], 1);
    }

    iterator end() {
        return Iterator<typename Vector::iterator>(data[size_val - 1], data[size_val - 1], 1);
    }

    iterator rbegin() {
        return Iterator<typename Vector::iterator>(data[size_val - 1], data[0], -1);
    }

    iterator rend() {
        return Iterator<typename Vector::iterator>(data[0], data[0], -1);
    }

    Vector() : size_val(0), empty_val(true) {
